function [TrainingTime, TestingTime, TrainingAccuracy, TestingAccuracy,TY] = elm_kernel1(P,T,TV_P,TV_T, Elm_Type, Regularization_coefficient, Kernel_type, Kernel_para)
...
T=train_data(:,1)'; %训练输出
P=train_data(:,2:size(train_data,2))'; %训练输入
clear train_data;                                   %   Release raw training data array

%%%%%%%%%%% Load testing dataset
test_data=load(TestingData_File);
TV.T=test_data(:,1)'; %测试输出
TV.P=test_data(:,2:size(test_data,2))'; %测试输入
clear test_data;      %   Release raw testing data array
%}

C = Regularization_coefficient; %正则化系数
NumberofTrainingData=size(P,2); %训练数据的样本数
NumberofTestingData=size(TV_P,2); %测试数据的样本数
...
%%%%%%%%%%% Training Phase %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tic; %tic 启动秒表计时器来测量性能。函数会记录执行 tic 命令时的内部时间。使用 toc 函数显示已用时间。
n = size(T,2);
...
tic;
Omega_test = kernel_matrix(P',Kernel_type, Kernel_para,TV_P');
...
%%%%%%%%%% Calculate training & testing accuracy (RMSE) for regression case
    ...
    TestingAccuracy=sqrt(mse(TV_T - TY))           
end
...
nb_data = size(Xtrain,1); %nb_data记录训练集的行数

%nargin：函数输入参数数目


if strcmp(kernel_type,'RBF_kernel'),   %高斯核                   
    %strcmp(s1,s2)比较两个字符串s1,s2是否相同相同继续执行，不同跳出执行同济循环。
    
    if nargin<4, %只有训练集
        XXh = sum(Xtrain.^2,2)*ones(1,nb_data); %矩阵每个元素求平方然后对行求和
...
elseif strcmp(kernel_type,'lin_kernel') %线性核
    if nargin<4,
        omega = Xtrain*Xtrain';
    else
        omega = Xtrain*Xt';
...
elseif strcmp(kernel_type,'poly_kernel')%多项式核
    if nargin<4,
